...
// packages/themes-default/src/default.js
var defaultThemeCss;
...
// packages/themes-default/src/tech-dark.js
var techDarkOverrides;
var init_tech_dark = __esm({
  "packages/themes-default/src/tech-dark.js"() {
    techDarkOverrides = `
/* ========================================================================= */
...
// packages/themes-default/src/paper.js
var paperOverrides;
...
// packages/themes-default/src/notion-like.js
var notionLikeOverrides;
var init_notion_like = __esm({
  "packages/themes-default/src/notion-like.js"() {
    notionLikeOverrides = `
/* ========================================================================= */
...
// packages/themes-default/src/runtime.js
var init_runtime = __esm({
  "packages/themes-default/src/runtime.js"() {
  }
});

// packages/themes-default/src/index.js
var themes;
var init_src = __esm({
  "packages/themes-default/src/index.js"() {
    init_default();
...
`);
                ...
                switch (bytes[i++]) {
                  ...
                  case 117: {
                    ...
                    for (let j = 0; j < 4; j++) {
                      c = bytes[i++];
                      code2 <<= 4;
                      if (c >= 48 && c <= 57) code2 |= c - 48;
                      else if (c >= 97 && c <= 102) code2 |= c + (10 - 97);
                      else if (c >= 65 && c <= 70) code2 |= c + (10 - 65);
                      else throwSyntaxError(bytes, --i);
                    }
                    value += fromCharCode(code2);
                    break;
                  }
    ...
    function createChannel(streamIn) {
      ...
      let afterClose = (error) => {
        ...
        for (let id in responseCallbacks) {
          responseCallbacks[id](text2, null);
        }
        responseCallbacks = {};
      };
    ...
    function extractErrorMessageV8(e, streamIn, stash, note, pluginName) {
      ...
      return { id: "", pluginName, text: text2, location, notes: note ? [note] : [], detail: stash ? stash.store(e) : -1 };
    }
    ...
    function convertOutputFiles({ path: path3, contents, hash }) {
      ...
      return {
        ...
        get text() {
          ...
          if (text2 === null || binary !== contents) {
            contents = binary;
            text2 = decodeUTF8(binary);
          }
          return text2;
        }
      };
...
// scripts/build-dist-lib.js
var build_dist_lib_exports = {};
__export(build_dist_lib_exports, {
  buildDist: () => buildDist,
...
async function buildDist({ outdir = join(projectRoot, "dist"), themeNames = Object.keys(themes) } = {}) {
...
var import_esbuild, projectRoot;
var init_build_dist_lib = __esm({
  "scripts/build-dist-lib.js"() {
    import_esbuild = __toESM(require_main(), 1);
    init_src();
    projectRoot = dirname(dirname(fileURLToPath(import.meta.url)));
  }
});
...
import { copyFile, mkdir as mkdir2, readFile as readFile2, readdir, writeFile as writeFile2 } from "node:fs/promises";
import { dirname as dirname2, join as join2, resolve as resolve2 } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";

// packages/ast/src/index.js
var RMD_AST_VERSION = "0.1.0";
function createRoot({ frontmatter = {}, warnings = [], children = [] } = {}) {
  return {
    type: "root",
...
function codeBlock(value, { lang = null, warnings = [], raw = value } = {}) {
  const node = {
    type: "code-block",
    lang,
    value
...
// packages/blocks-core/src/index.js
...
function parseCoreBlock(block2, parseChildren) {
  let node;
  switch (block2.name) {
    case "chart":
      node = parseChart(block2);
      break;
    case "grid":
      node = parseGrid(block2, parseChildren);
      break;
    case "callout":
      node = parseCallout(block2, parseChildren);
      break;
    case "slider":
      node = parseSlider(block2);
      break;
    case "export":
      node = parseExport(block2);
      break;
    case "flow":
      node = parseFlow(block2);
      break;
    case "diff":
      node = parseDiff(block2);
      break;
    case "tabs":
      node = parseTabs(block2, parseChildren);
      break;
    case "timeline":
      node = parseTimeline(block2, parseChildren);
      break;
    case "kanban":
      node = parseKanban(block2, parseChildren);
      break;
    case "details":
      node = parseDetails(block2, parseChildren);
      break;
    case "carousel":
      node = parseCarousel(block2, parseChildren);
      break;
    case "embed":
      node = parseEmbed(block2);
      break;
    case "math":
      node = parseMath(block2);
      break;
    default:
      node = unknownBlock(block2);
      break;
  }
  if (block2.warnings?.length > 0) {
    node.warnings = [...node.warnings ?? [], ...block2.warnings];
  }
  return node;
}
function unknownBlock(block2) {
  return codeBlock(block2.raw, {
    lang: "rmd",
    raw: block2.raw,
    warnings: [`unknown-block: ${block2.name}`]
  });
}
function parseChart(block2) {
  const { positional, attrs } = parseAttrs(block2.attrText);
  ...
  const data = [];
  const warnings = [];
  for (const line of nonEmptyLines(block2.content)) {
...
function parseGrid(block2, parseChildren) {
  const { positional, attrs } = parseAttrs(block2.attrText);
  ...
  const isInvalid = Array.isArray(columns) ? columns.some((c) => !Number.isInteger(c) || c < 1 || c > 12) : !Number.isInteger(columns) || columns < 1 || columns > 12;
  if (isInvalid) {
    return codeBlock(block2.raw, {
      lang: "rmd",
      raw: block2.raw,
  ...
  const warnings = [];
  let cellSources = splitGridCells(block2.content);
...
function parseCallout(block2, parseChildren) {
  const { positional, attrs } = parseAttrs(block2.attrText);
  ...
  return withOptionalFields({
    ...
    title: attrs.title ?? null,
    children: parseChildren(block2.content)
  }, { unknownAttrs });
}
function parseSlider(block2) {
  ...
  const step = attrs.step === void 0 ? 1 : Number(attrs.step);
  let defaultValue = min;
  if (attrs.default !== void 0) {
...
function parseExport(block2) {
  const { attrs } = parseAttrs(block2.attrText);
  const unknownAttrs = pickUnknownAttrs(attrs, ["label", "format"]);
  const format2 = ["text", "markdown", "json"].includes(attrs.format) ? attrs.format : "text";
  const references = [...new Set([...block2.content.matchAll(/{{\s*([a-zA-Z_][a-zA-Z0-9_]*)(?:\s*\|[^}]*)?}}/g)].map((match2) => match2[1]))];
  return withOptionalFields({
    type: "export",
    label: attrs.label ?? "\u590D\u5236",
    format: format2,
    template: block2.content.replace(/\n$/, ""),
    references
  }, { unknownAttrs });
}
function parseFlow(block2) {
  const { attrs } = parseAttrs(block2.attrText);
...
function parseDiff(block2) {
  const { attrs } = parseAttrs(block2.attrText);
  const unknownAttrs = pickUnknownAttrs(attrs, ["lang", "title"]);
  const lines = block2.content.split(/\r?\n/).filter((line) => line.length > 0).map((line) => {
    const first = line[0];
...
function parseTabs(block2, parseChildren) {
  const { attrs } = parseAttrs(block2.attrText);
...
function parseTimeline(block2, parseChildren) {
  const { attrs } = parseAttrs(block2.attrText);
  const unknownAttrs = pickUnknownAttrs(attrs, ["direction"]);
  const items = [];
  let current = null;
...
function parseCarousel(block2, parseChildren) {
  const { attrs } = parseAttrs(block2.attrText);
  const unknownAttrs = pickUnknownAttrs(attrs, ["autoplay", "interval"]);
  const cellSources = splitGridCells(block2.content);
  return withOptionalFields({
    type: "carousel",
    autoplay: attrs.autoplay === "true",
    interval: attrs.interval ? Number(attrs.interval) : 3e3,
    items: cellSources.map((cell) => parseChildren(cell))
  }, { unknownAttrs });
}
function parseEmbed(block2) {
  const { attrs } = parseAttrs(block2.attrText);
...
function parseMath(block2) {
  return {
    type: "math",
    value: block2.content.trim()
  };
}
...
// packages/parser-core/src/index.js
var markdown = new lib_default("commonmark", {
  html: true,
  linkify: false,
  typographer: false
});
function parse(source) {
  ...
  return createRoot({
    frontmatter,
    warnings,
    children: parseBlocks(body, { sliderNames: /* @__PURE__ */ new Set() })
  });
}
function parseBlocks(source, context) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const fence2 = matchMarkdownFence(line);
    if (markdownFence) {
      markdownLines.push(line);
      if (fence2 && fence2.marker === markdownFence.marker && fence2.length >= markdownFence.length) {
        markdownFence = null;
      }
      continue;
    }
    if (fence2) {
      markdownFence = fence2;
      markdownLines.push(line);
      continue;
    }
    ...
    if (blockStart) {
      flushMarkdown();
      const block2 = collectBlock(lines, index, blockStart);
      index = block2.endIndex;
      const node = parseCoreBlock(block2, (childSource) => parseBlocks(childSource, context));
    ...
    switch (token.type) {
      case "heading_open": {
        ...
        nodes.push({
          ...
          children: inline2?.type === "inline" ? parseInlineTokens(inline2.children ?? []) : []
        });
        index += 3;
        break;
      }
...
function collectBlock(lines, startIndex, blockStart) {
  ...
  for (let index = startIndex + 1; index < lines.length; index += 1) {
    const fence2 = matchMarkdownFence(lines[index]);
    if (markdownFence) {
  ...
  if (!closed) {
    block2.raw = lines.slice(startIndex).join("\n");
  }
  return block2;
}
function matchMarkdownFence(line) {
  const match2 = line.match(/^\s*(`{3,}|~{3,})/);
  if (!match2) {
    return null;
  }
  return {
...
function parseSimpleYaml(raw) {
  ...
  for (const line of raw.split("\n")) {
    ...
    const key = match2[1];
    const value = stripYamlQuotes(match2[2].trim());
    result[key] = value;
  }
  return result;
}
...
function renderToString(input, options = {}) {
  ...
  const title = ast.frontmatter?.title ? escapeHtml2(ast.frontmatter.title) : "";
  return [
    "<!doctype html>",
...
function renderFragmentToString(input) {
  const ast = typeof input === "string" ? parse(input) : input;
...
function renderCodeBlock(node) {
  ...
  return `<pre class="rmd-code"${warningAttr}><code${langClass}>${escapeHtml2(node.value)}</code></pre>`;
}
...
function renderChart(node) {
  ...
  const bars = node.data.map((point, index) => {
    ...
    return [
      ...
      `<text x="${x + 18}" y="170" text-anchor="middle">${escapeHtml2(point.label)}</text>`,
      `<text x="${x + 18}" y="${y - 8}" text-anchor="middle">${escapeHtml2(String(value))}</text>`,
      "</g>"
    ].join("");
  }).join("");
  ...
  return [
    ...
    node.title ? `<h3 class="rmd-block-title">${escapeHtml2(node.title)}</h3>` : "",
    `<svg role="img" viewBox="0 0 ${Math.max(240, node.data.length * 64 + 48)} 190" class="rmd-chart-svg">`,
    node.title ? `<title>${escapeHtml2(node.title)}</title>` : "<title>Chart</title>",
    `<desc>${escapeHtml2(chartDescription(node))}</desc>`,
    bars,
    "</svg>",
    "</section>"
  ].filter(Boolean).join("");
...
function renderCallout(node) {
  return [
    ...
    node.title ? `<h3 class="rmd-callout-title">${escapeHtml2(node.title)}</h3>` : "",
...
function renderSlider(node) {
  ...
  return [
    ...
    `<label for="rmd-slider-${escapeAttr(node.name)}">${escapeHtml2(node.label)}</label>`,
    ...
    `<output>${escapeHtml2(defaultStr)}${node.unit ? ` ${escapeHtml2(node.unit)}` : ""}</output>`,
    "</div>"
  ].join("");
}
function renderExport(node) {
  return [
    ...
    `<button type="button">${escapeHtml2(node.label)}</button>`,
    `<pre><code>${escapeHtml2(node.template)}</code></pre>`,
    "</section>"
  ].join("");
}
function renderFlow(node) {
  const edges = node.edges.map((edge) => `<li><span>${escapeHtml2(edge.from)}</span> <span aria-hidden="true">\u2192</span> <span>${escapeHtml2(edge.to)}</span></li>`).join("");
  return [
...
function renderDiff(node) {
  const title = node.title ? `<h3 class="rmd-block-title">${escapeHtml2(node.title)}</h3>` : "";
  const lines = node.lines.map((line) => {
    const prefix = line.kind === "add" ? "+" : line.kind === "remove" ? "-" : " ";
    const note = line.note ? ` <mark>${escapeHtml2(line.note)}</mark>` : "";
    return `<div class="rmd-diff-line rmd-diff-${line.kind}"><code>${escapeHtml2(`${prefix} ${line.text}`)}</code>${note}</div>`;
  }).join("");
  return [
...
function renderTabs(node) {
  const buttons = node.panels.map((panel) => {
    ...
    return `<button type="button" role="tab" aria-selected="${selected}" data-label="${escapeAttr(panel.label)}">${escapeHtml2(panel.label)}</button>`;
  }).join("");
...
function renderTimeline(node) {
  const items = node.items.map((item) => {
    ...
    const titleHtml = item.title ? `<h4>${escapeHtml2(item.title)}</h4>` : "";
    return [
      `<li class="rmd-timeline-item${statusClass}">`,
      `<div class="rmd-timeline-time">${escapeHtml2(item.time)}</div>`,
...
function renderDetails(node) {
  ...
  return [
    ...
    `<summary>${escapeHtml2(node.title)}</summary>`,
...
function renderMath(node) {
  return [
    ...
    `<pre><code class="language-latex">${escapeHtml2(node.value)}</code></pre>`,
    `</section>`
  ].join("");
}
function renderUnknownNode(node) {
  return `<pre class="rmd-unknown-node" data-rmd-node="${escapeAttr(node.type)}"><code>${escapeHtml2(JSON.stringify(node, null, 2))}</code></pre>`;
}
...
// packages/runtime/src/inline-runtime.js
var inlineRuntimeScript = `
(() => {
...
init_src();
function buildHtml(source, options = {}) {
...
function buildSelfContained(source, ast, { theme }) {
  const html = renderToString(ast, { theme });
...
${inlineRuntimeScript}
  ...
  return html.replace("</head>", `<style data-rmd-theme-inline="${escapeAttr2(theme)}">
${css}
</style>
</head>`).replace("</body>", `${sourceTag}
${runtimeTag}
</body>`);
}
function buildCdn(source, { theme, version, cdnBase }) {
  ...
  return [
    ...
    '<meta charset="utf-8">',
    '<meta name="viewport" content="width=device-width, initial-scale=1">',
    `<script src="${escapeAttr2(`${root}/rmd.min.js`)}"></script>`,
    `<link rel="stylesheet" href="${escapeAttr2(`${root}/themes/${theme}.css`)}">`,
    "</head>",
    "<body>",
    '<div id="rmd-root"></div>',
...
function buildSplit(source, { theme, scriptPath = "./rmd.min.js", themePath }) {
  ...
  return [
    ...
    '<meta charset="utf-8">',
    '<meta name="viewport" content="width=device-width, initial-scale=1">',
    `<script src="${escapeAttr2(scriptPath)}"></script>`,
    `<link rel="stylesheet" href="${escapeAttr2(cssPath)}">`,
    "</head>",
    "<body>",
    '<div id="rmd-root"></div>',
    `<script type="application/rmd">${escapeScript(source)}</script>`,
...
function validateRoot(ast, errors) {
  if (!isObject2(ast)) {
    add2(errors, "", "AST must be an object");
    return;
  }
  if (ast.type !== "root") {
    add2(errors, "/type", 'root type must be "root"');
  }
  if (!isSemver(ast.version)) {
    add2(errors, "/version", "version must be a semver string");
  }
  if (!isObject2(ast.frontmatter)) {
    add2(errors, "/frontmatter", "frontmatter must be an object");
  }
  if (!Array.isArray(ast.warnings)) {
    add2(errors, "/warnings", "warnings must be an array");
  }
  if (!Array.isArray(ast.children)) {
    add2(errors, "/children", "children must be an array");
    return;
  }
  const sliderNames = /* @__PURE__ */ new Set();
...
function validateBlockNode(node, path, errors, context) {
  if (!isObject2(node)) {
    add2(errors, path, "node must be an object");
    return;
  }
  if (typeof node.type !== "string") {
    add2(errors, `${path}/type`, "node type must be a string");
    return;
  }
  if (node.warnings !== void 0 && !Array.isArray(node.warnings)) {
    add2(errors, `${path}/warnings`, "warnings must be an array when present");
  }
  if (node.unknownAttrs !== void 0 && !isStringRecord(node.unknownAttrs)) {
    add2(errors, `${path}/unknownAttrs`, "unknownAttrs must be a string record when present");
  }
  switch (node.type) {
    case "heading":
    ...
    case "math":
      ...
      break;
    default:
      add2(errors, `${path}/type`, `unsupported block node type: ${node.type}`);
  }
}
function validateHeading(node, path, errors) {
  if (!Number.isInteger(node.level) || node.level < 1 || node.level > 6) {
    add2(errors, `${path}/level`, "heading level must be an integer from 1 to 6");
  }
  validateInlineChildren(node, path, errors);
}
function validateInlineChildren(node, path, errors) {
  if (!Array.isArray(node.children)) {
    add2(errors, `${path}/children`, "children must be an array");
    return;
  }
...
function validateInlineNode(node, path, errors) {
  if (!isObject2(node)) {
    add2(errors, path, "inline node must be an object");
    return;
  }
  switch (node.type) {
    case "text":
    ...
    case "inline-html":
      ...
      break;
    default:
      add2(errors, `${path}/type`, `unsupported inline node type: ${node.type}`);
  }
}
function validateCodeBlock(node, path, errors) {
  if (node.lang !== null && typeof node.lang !== "string") {
    add2(errors, `${path}/lang`, "code-block lang must be string or null");
  }
  requireString(node.value, `${path}/value`, errors);
}
function validateList(node, path, errors, context) {
  if (typeof node.ordered !== "boolean") {
    add2(errors, `${path}/ordered`, "list ordered must be a boolean");
  }
  if (node.start !== null && (!Number.isInteger(node.start) || node.start < 1)) {
    add2(errors, `${path}/start`, "list start must be a positive integer or null");
  }
  if (typeof node.tight !== "boolean") {
    add2(errors, `${path}/tight`, "list tight must be a boolean");
  }
  if (!Array.isArray(node.children)) {
    add2(errors, `${path}/children`, "list children must be an array");
    return;
  }
...
function validateListItem(node, path, errors, context) {
  if (node.type !== "list-item") {
    add2(errors, `${path}/type`, "list children must be list-item nodes");
  }
  if (node.checked !== null && typeof node.checked !== "boolean") {
    add2(errors, `${path}/checked`, "list item checked must be boolean or null");
  }
  validateBlockChildren(node, path, errors, context);
}
function validateChart(node, path, errors) {
  if (!["bar", "line", "pie", "scatter", "radar", "area", "donut", "heatmap"].includes(node.chartType)) {
    add2(errors, `${path}/chartType`, "chartType must be a valid type");
  }
  if (node.title !== null && typeof node.title !== "string") {
    add2(errors, `${path}/title`, "title must be string or null");
  }
  if (!["primary", "secondary", "none"].includes(node.emphasis)) {
    add2(errors, `${path}/emphasis`, "emphasis must be primary, secondary, or none");
  }
  if (!Array.isArray(node.data) || node.data.length === 0) {
    add2(errors, `${path}/data`, "chart data must contain at least one point");
    return;
  }
  ...
  node.data.forEach((point, index) => {
    ...
    if (!Array.isArray(point?.values) || point.values.length === 0 || point.values.some((value) => typeof value !== "number" || Number.isNaN(value))) {
      add2(errors, `${path}/data/${index}/values`, "chart values must be a non-empty number array");
      return;
    }
    if (point.values.length !== seriesLength) {
      add2(errors, `${path}/data/${index}/values`, "all chart series must have the same length");
    }
  });
  if (node.chartType === "pie" && seriesLength !== 1) {
    add2(errors, `${path}/data`, "pie charts must have exactly one series");
  }
}
function validateGrid(node, path, errors, context) {
  if (Array.isArray(node.columns)) {
    if (node.columns.some((c) => !Number.isInteger(c) || c < 1 || c > 12)) {
      add2(errors, `${path}/columns`, "grid columns array must contain integers from 1 to 12");
    }
  } else if (!Number.isInteger(node.columns) || node.columns < 1 || node.columns > 12) {
    add2(errors, `${path}/columns`, "grid columns must be an integer from 1 to 12");
  }
  if (!["sm", "md", "lg"].includes(node.gap)) {
    add2(errors, `${path}/gap`, "grid gap must be sm, md, or lg");
  }
  if (!Array.isArray(node.cells) || node.cells.length > 12) {
    add2(errors, `${path}/cells`, "grid cells must be an array with at most 12 entries");
    return;
  }
  node.cells.forEach((cell, cellIndex) => {
    if (!Array.isArray(cell)) {
      add2(errors, `${path}/cells/${cellIndex}`, "grid cell must be an array");
      return;
    }
    cell.forEach((child, childIndex) => {
      if (child?.type === "grid") {
        add2(errors, `${path}/cells/${cellIndex}/${childIndex}`, "grid cannot contain nested grid nodes in v0.1");
      }
...
function validateCallout(node, path, errors, context) {
  if (!["info", "tip", "warning", "danger", "success"].includes(node.kind)) {
    add2(errors, `${path}/kind`, "callout kind is invalid");
  }
  if (node.title !== null && typeof node.title !== "string") {
    add2(errors, `${path}/title`, "title must be string or null");
  }
...
function validateSlider(node, path, errors, context) {
  if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(node.name ?? "")) {
    add2(errors, `${path}/name`, "slider name must be a valid identifier");
  } else if (!node.duplicate && context.sliderNames.has(node.name)) {
    add2(errors, `${path}/name`, "slider name must be unique");
  ...
  if (typeof node.min === "number" && typeof node.max === "number" && node.max <= node.min) {
    add2(errors, `${path}/max`, "slider max must be greater than min");
  }
  if (typeof node.step === "number" && node.step <= 0) {
    add2(errors, `${path}/step`, "slider step must be greater than 0");
  }
  if (Array.isArray(node.default)) {
    if (node.default.some((v) => typeof v !== "number" || typeof node.min !== "number" || typeof node.max !== "number" || v < node.min || v > node.max)) {
      add2(errors, `${path}/default`, "slider default values must be within min and max");
    }
    ...
    add2(errors, `${path}/default`, "slider default must be within min and max");
  }
...
function validateExport(node, path, errors) {
  ...
  if (!["text", "markdown", "json"].includes(node.format)) {
    add2(errors, `${path}/format`, "export format must be text, markdown, or json");
  }
  ...
  if (!Array.isArray(node.references) || node.references.some((name) => !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(name))) {
    add2(errors, `${path}/references`, "references must be identifier strings");
  }
}
function validateFlow(node, path, errors) {
  if (!["lr", "tb"].includes(node.direction)) {
    add2(errors, `${path}/direction`, "flow direction must be lr or tb");
  }
  if (!Array.isArray(node.nodes)) {
    add2(errors, `${path}/nodes`, "flow nodes must be an array");
    return;
  }
  if (!Array.isArray(node.edges) || node.edges.length === 0) {
    add2(errors, `${path}/edges`, "flow edges must be a non-empty array");
    return;
  }
  const nodeIds = /* @__PURE__ */ new Set();
  ...
  node.edges.forEach((edge, index) => {
    ...
    if (typeof edge?.from === "string" && !nodeIds.has(edge.from)) {
      add2(errors, `${path}/edges/${index}/from`, "edge from must reference a declared node");
    }
    if (typeof edge?.to === "string" && !nodeIds.has(edge.to)) {
      add2(errors, `${path}/edges/${index}/to`, "edge to must reference a declared node");
    }
  });
}
function validateDiff(node, path, errors) {
  if (node.lang !== null && typeof node.lang !== "string") {
    add2(errors, `${path}/lang`, "diff lang must be string or null");
  }
  if (node.title !== null && typeof node.title !== "string") {
    add2(errors, `${path}/title`, "diff title must be string or null");
  }
  if (!Array.isArray(node.lines)) {
    add2(errors, `${path}/lines`, "diff lines must be an array");
    return;
  }
  node.lines.forEach((line, index) => {
    if (!["add", "remove", "context"].includes(line?.kind)) {
      add2(errors, `${path}/lines/${index}/kind`, "diff line kind is invalid");
    }
    requireString(line?.text, `${path}/lines/${index}/text`, errors);
    if (line?.note !== null && typeof line?.note !== "string") {
      add2(errors, `${path}/lines/${index}/note`, "diff line note must be string or null");
    }
  });
}
function validateTabs(node, path, errors, context) {
  if (!Array.isArray(node.panels) || node.panels.length === 0) {
    add2(errors, `${path}/panels`, "tabs panels must contain at least one panel");
    return;
  }
  ...
  node.panels.forEach((panel, index) => {
    ...
    if (typeof panel?.label === "string") {
      if (labels.has(panel.label)) {
        add2(errors, `${path}/panels/${index}/label`, "tabs panel labels must be unique");
      }
      labels.add(panel.label);
    }
    if (!Array.isArray(panel?.children)) {
      add2(errors, `${path}/panels/${index}/children`, "tabs panel children must be an array");
      return;
    }
    panel.children.forEach((child, childIndex) => {
      if (child?.type === "tabs") {
        add2(errors, `${path}/panels/${index}/children/${childIndex}`, "tabs cannot contain nested tabs in v0.1");
      }
      validateBlockNode(child, `${path}/panels/${index}/children/${childIndex}`, errors, context);
    });
  });
  if (!labels.has(node.default)) {
    add2(errors, `${path}/default`, "tabs default must match a panel label");
  }
}
...
function validateTimeline(node, path, errors, context) {
  if (!["horizontal", "vertical"].includes(node.direction)) {
    add2(errors, `${path}/direction`, "timeline direction must be horizontal or vertical");
  }
  if (!Array.isArray(node.items)) {
    add2(errors, `${path}/items`, "timeline items must be an array");
    return;
  }
  node.items.forEach((item, index) => {
    requireString(item?.time, `${path}/items/${index}/time`, errors);
    if (item.title !== null && typeof item.title !== "string") {
      add2(errors, `${path}/items/${index}/title`, "title must be string or null");
    }
    if (!["default", "success", "warning", "danger", "pending"].includes(item.status)) {
      add2(errors, `${path}/items/${index}/status`, "status must be a valid enum");
    }
    if (!Array.isArray(item.children)) {
      add2(errors, `${path}/items/${index}/children`, "children must be an array");
    } else {
...
function validateKanban(node, path, errors, context) {
  if (!Array.isArray(node.columns)) {
    add2(errors, `${path}/columns`, "kanban columns must be an array");
    return;
  }
  node.columns.forEach((col, index) => {
    requireString(col?.title, `${path}/columns/${index}/title`, errors);
    if (!Array.isArray(col.children)) {
      add2(errors, `${path}/columns/${index}/children`, "children must be an array");
    } else {
...
function validateDetails(node, path, errors, context) {
  ...
  if (typeof node.open !== "boolean") {
    add2(errors, `${path}/open`, "open must be a boolean");
  }
  validateBlockChildren(node, path, errors, context);
}
function validateCarousel(node, path, errors, context) {
  if (typeof node.autoplay !== "boolean") {
    add2(errors, `${path}/autoplay`, "autoplay must be a boolean");
  }
  requireNumber(node.interval, `${path}/interval`, errors);
  if (!Array.isArray(node.items)) {
    add2(errors, `${path}/items`, "items must be an array");
    return;
  }
  node.items.forEach((item, index) => {
    if (!Array.isArray(item)) {
      add2(errors, `${path}/items/${index}`, "carousel item must be an array");
    } else {
...
function validateEmbed(node, path, errors) {
  ...
  if (node.aspectRatio !== null && typeof node.aspectRatio !== "string") {
    add2(errors, `${path}/aspectRatio`, "aspectRatio must be string or null");
  }
}
function validateMath(node, path, errors) {
  requireString(node.value, `${path}/value`, errors);
}
function add2(errors, path, message) {
  errors.push({ path, message });
}
function requireString(value, path, errors) {
  if (typeof value !== "string") {
    add2(errors, path, "must be a string");
  }
}
function requireNumber(value, path, errors) {
  if (typeof value !== "number" || Number.isNaN(value)) {
    add2(errors, path, "must be a number");
  }
}
...
// packages/cli/src/preview-server.js
import { readFile } from "node:fs/promises";
import { createServer } from "node:http";
import { resolve } from "node:path";
async function createPreviewServer(file, options = {}) {
...
async function installSkillForPlatform(platform, options, io) {
  ...
  const targetDir = resolve2(process.cwd(), target.root, "skills", SKILL_NAME);
...
function resolveInitPlatforms(rawAi) {
  ...
  const values = String(rawAi).split(",").map((item) => item.trim()).filter(Boolean);
...
function detectPlatform() {
  if (existsSync(resolve2(process.cwd(), ".codex"))) return "codex";
  if (existsSync(resolve2(process.cwd(), ".claude"))) return "claude";
  return null;
}
...
async function copyDirectory(sourceDir, targetDir, options) {
  ...
  for (const entry of entries) {
    ...
    const source = join2(sourceDir, entry.name);
...
async function runParse(args, io) {
  ...
  const source = await readFile2(resolve2(file), "utf8");
  const ast = parse(source);
...
async function runBuild(args, io) {
  ...
  if (flags.out) {
    const outPath = resolve2(flags.out);
    await mkdir2(dirname2(outPath), { recursive: true });
    await writeFile2(outPath, html, "utf8");
    if (flags.mode === "split") {
...
async function writeSplitAssets(outPath, theme) {
  const { buildDist: buildDist2 } = await Promise.resolve().then(() => (init_build_dist_lib(), build_dist_lib_exports));
  const outDir = dirname2(outPath);
  await buildDist2({ outdir: outDir, themeNames: [theme] });
}
...
if (process.argv[1]) {
  try {
    isEntrypoint = realpathSync(process.argv[1]) === fileURLToPath2(import.meta.url);
  } catch (e) {
    isEntrypoint = resolve2(process.argv[1]) === fileURLToPath2(import.meta.url);
  }
}
if (isEntrypoint) {
